...
cls
echo %cdc%----------------------------------------------------------------%cr%
echo %cdb%   _____           _       __         %cr%      %cdy%Made By:%cr% %cdb%cylvin%cr%
echo %cdb%  / ___/__________(_)___  / /___  __  %cr%%cbg%github.com/cyl-vin/Scripty%cr%
echo %cdb%  \__ \/ ___/ ___/ / __ \/ __/ / / /  %cr%
echo %cdb% ___/ / /__/ /  / / /_/ / /_/ /_/ /   %cr%
echo %cdb%/____/\___/_/  /_/ .___/\__/\__, /    %cr%
echo %cdb%                /_/        /____/%cr% %blb%%cdr%v0.6%cr%
echo %cdc%----------------------------------------------------------------%cr%
...
echo %cdy%4.)%cr% Generate a password of any length (includes special chars.)
echo %cdy%5.)%cr% Terminal color test
echo %cdy%0.)%cr% %cdr%EXIT AND CLOSE SCRIPTY%cr%
echo %cdc%----------------------------------------------------------------%cr%
echo Type %cdr%Exit%cr% to close Scripty from anywhere
echo Type %cdc%Leave%cr% anywhere to come back to this menu
echo %cdc%----------------------------------------------------------------%cr%
set /p "maininput=%cbm%Main Menu%cr%:-$ "
...
echo %cdc%----------------------------------------------------------------%cr%
echo Type %cdr%x%cr% to sort all files by extension into sperate folders
echo Type %cdg%m%cr% to sort most files by type into seperate folders
echo %cdy%(typing 'm' combines below options into one command)%cr%
echo ----------------------------------------------------------------
echo Type %cdy%video%cr% to move video files into "Videos"
echo Type %cdy%audio%cr% to move audio files into "Audio"
echo Type %cdy%image%cr% to move image files into "Images"
echo Type %cdy%text%cr% to move text-based files into "Documents"
echo Type %cdy%present%cr% to move presentation files into "Presentation"
echo Type %cdy%exe%cr% to move executable files into "Executables"
echo Type %cdy%disc%cr% to move Disc/Media files into "Disc_Images"
echo Type %cdy%zip%cr% to move compressed file types into "Compressed_Files"
echo Type %cdc%Leave%cr% and hit %cdg%ENTER%cr% to go to the Main Menu
echo Type %cdr%Exit%cr% and hit %cdg%ENTER%cr% to leave Scripty
echo %cdc%----------------------------------------------------------------%cr%
set /p "organizer=%cby%Batch%cr%:-$ "
...
echo %cdb%                /_/        /____/%cr% %blb%%cdr%v0.6%cr%
echo %cdc%----------------------------------------------------------------%cr%
echo Rename file extensions %cdr%(Finished)%cr%
echo %cdc%----------------------------------------------------------------%cr%
echo Type %cdy%Change%cr% to %cdg%CHANGE%cr% File extensions
echo Type %cdy%Restore%cr% to %cdr%REVERT%cr% File extensions
echo Type %cdc%Leave%cr% to %cdc%LEAVE%cr% this program
echo Type %cdr%Exit%cr% to close Scripty
echo %cdc%----------------------------------------------------------------%cr%
set /p "rfe=%cdy%Command%cr%:-$ "
...
echo %cdb%                /_/        /____/%cr% %blb%%cdr%v0.6%cr%
echo %cdc%----------------------------------------------------------------%cr%
echo Type %cdy%length%cr% to change the length of the password
echo Type %cdy%gen%cr% to generate the password
echo Type %cdc%leave%cr% to go back to the main menu
echo Current Password Length: %passlength%
echo %cdc%----------------------------------------------------------------%cr%
echo %cdr%Generated Password%cr%: %passresult%
echo %cdc%----------------------------------------------------------------%cr%
echo %cdr%%bold%%u%REMEMBER TO WRITE DOWN THE PASSWORD%cr%
echo %cdc%----------------------------------------------------------------%cr%

set /p "passgenerator=%cdg%Password%cr%:-$ "
